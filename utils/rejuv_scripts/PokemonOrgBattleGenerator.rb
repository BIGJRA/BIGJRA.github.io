...
class NonlegendaryRestriction
  def isValid?(pkmn)
    compat10=$pkmn_dex[pkmn][13][0]
    compat11=$pkmn_dex[pkmn][13][1]
    genderbyte=$pkmn_dex[pkmn][7]
...
def pbRandomPokemonFromRule(rule,trainer)
  ...
  begin
    ...
    if isConst?(item,PBItems,:BLACKSLUDGE)
      if !isConst?($pkmn_dex[species][3],PBTypes,:POISON) && !isConst?($pkmn_dex[species][4],PBTypes,:POISON)
        item=getID(PBItems,:LEFTOVERS)
      end
    end
