  ...
  loop do
    ...
    if Input.trigger?(Input::C)  && animations.length>0
      ...
      if cmd2==0 # Load Animation
        ...
        animations[cmdwin.index].name = Kernel.pbMessageFreeText(_INTL("What code would you like to run?"),"",false,999,500)
        cmdwin.refresh
      elsif cmd2==2 # Delete
...
def animationEditorMain(animation)
  ...
  # Right hand menu
  sidewin=ControlWindow.new(640,0,160,502)
...
def pbAnimationEditor
  pbBGMStop()
  if !$pkmn_animations
    $pkmn_animations=tryLoadData("Data/PkmnAnimations.rxdata")
  end
  animation = $pkmn_animations
...
def pbConvertAnimsToNewFormat
  ...
  $pkmn_animations = animation if count>0
