# List of functions that exist in the code but don't have a menu option:
# pbCelRotateFlip
# pbFlipAroundPoint
...
class Checkbox < Button
  ...
  def setvalue()
    if @setProc.nil?
      print "something went wrong ", caller
      return
    end
    @setProc.call(curvalue)
  end
...
class Slider < UIControl
  ...
  def initialize(label,minvalue,maxvalue,curval,setblock=nil)
    super(label,setblock)
    @minvalue=minvalue
    @maxvalue=maxvalue
    @curvalue=curval
...
class OptionalSlider < Slider
  def initialize(label,minvalue,maxvalue,curvalue,setblock=nil)
...
class TextSlider < UIControl
  ...
  def initialize(label,options,curval,setblock=nil)
    super(label,setblock)
    @label=label
    @options=options
    @minvalue=0
...
class ControlWindow < SpriteWindow_Base
  ...
  def addLabel(label,setproc=nil)
    return addControl(Label.new(label,setproc))
  end

  def addButton(label,setproc=nil)
    return addControl(Button.new(label,setproc))
  end

  def addSlider(label,minvalue,maxvalue,curvalue,setproc=nil)
    return addControl(Slider.new(label,minvalue,maxvalue,curvalue,setproc))
  end

  def addOptionalSlider(label,minvalue,maxvalue,curvalue,setproc=nil)
    return addControl(OptionalSlider.new(label,minvalue,maxvalue,curvalue,setproc))
  end

  def addTextSlider(label,options,curvalue,setproc=nil)
    return addControl(TextSlider.new(label,options,curvalue,setproc))
  end

  def addOptionalTextSlider(label,options,curvalue,setproc=nil)
    return addControl(OptionalTextSlider.new(label,options,curvalue,setproc))
  end

  def addCheckbox(label,setproc=nil)
    return addControl(Checkbox.new(label,setproc))
  end
  ...
  def updateInput
    ...
    if mousepos && pbSpriteHitTest(self,mousepos[0],mousepos[1],false,true)
      if Input.triggerex?(Input::LeftMouseKey) # Left mouse button
        selectedcel=-1
        usealpha=(Input.triggerex?(:LALT)) ? true : false 
    ...
    if currentFrame && !@selecting && (Input.triggerex?(:TAB) || Input.repeatex?(:TAB))
    ...
    if cel
      ...
      if !self.locked?(@currentcel)
        if Input.repeat?(Input::UP)
          increment=(Input.triggerex?(:LALT)) ? 2 : 8
...
################################################################################
# Pop-up menus for individual cels (and a couple for the batch editor)
################################################################################
def pbChooseNum(cel)
  ret=cel
...
def pbSetToneBatch(tonehash)
  sliderwin2=ControlWindow.new(0,0,320,320)
  sliderwin2.z=99999
  sliderwin2.addLabel(_INTL("Note: Pressing cancel will"))
  sliderwin2.addLabel(_INTL("clear previously OK'd changes"))
  sliderwin2.addSpace
  sliderwin2.addOptionalSlider(_INTL("Red Offset:"),-255,255,tonehash["colortonered"])
  sliderwin2.addOptionalSlider(_INTL("Green Offset:"),-255,255,tonehash["colortonegreen"])
  sliderwin2.addOptionalSlider(_INTL("Blue Offset:"),-255,255,tonehash["colortoneblue"])
  sliderwin2.addOptionalSlider(_INTL("Gray Tone:"),0,255,tonehash["colortonegray"])
  okbutton=sliderwin2.addButton(_INTL("OK"))
  cancelbutton=sliderwin2.addButton(_INTL("Cancel"))
  loop do
    Graphics.update
    ...
    sliderwin2.update
    if sliderwin2.changed?(okbutton)
      if sliderwin2.value(3)
        tonehash["toneredchange"]=true
        tonehash["colortonered"]=sliderwin2.value(3)
      else
        tonehash["toneredchange"]=false
      end
      if sliderwin2.value(4)
        tonehash["tonegreenchange"]=true
        tonehash["colortonegreen"]=sliderwin2.value(4)
      else
        tonehash["tonegreenchange"]=false
      end
      if sliderwin2.value(5)
        tonehash["tonebluechange"]=true
        tonehash["colortoneblue"]=sliderwin2.value(5)
      else
        tonehash["tonebluechange"]=false
      end
      if sliderwin2.value(6)
        tonehash["tonegraychange"]=true
        tonehash["colortonegray"]=sliderwin2.value(6)
      else
        tonehash["tonegraychange"]=false
      end
      break
    end
    if sliderwin2.changed?(cancelbutton) || Input.trigger?(Input::B)
...
def pbSetFlashBatch(blendhash)
  sliderwin2=ControlWindow.new(0,0,320,320)
  sliderwin2.z=99999
  sliderwin2.addLabel(_INTL("Note: Pressing cancel will"))
  sliderwin2.addLabel(_INTL("clear previously OK'd changes"))
  sliderwin2.addSpace
  sliderwin2.addOptionalSlider(_INTL("Red:"),0,255,blendhash["blendcolorred"])
  sliderwin2.addOptionalSlider(_INTL("Green:"),0,255,blendhash["blendcolorgreen"])
  sliderwin2.addOptionalSlider(_INTL("Blue:"),0,255,blendhash["blendcolorblue"])
  sliderwin2.addOptionalSlider(_INTL("Alpha:"),0,255,blendhash["blendcoloralpha"])
  okbutton=sliderwin2.addButton(_INTL("OK"))
  cancelbutton=sliderwin2.addButton(_INTL("Cancel"))
  loop do
    Graphics.update
    ...
    if sliderwin2.changed?(okbutton)
      if sliderwin2.value(3)
        blendhash["blendredchange"]=true
        blendhash["blendcolorred"]=sliderwin2.value(3)
      else
        blendhash["blendredchange"]=false
      end
      if sliderwin2.value(4)
        blendhash["blendgreenchange"]=true
        blendhash["blendcolorgreen"]=sliderwin2.value(4)
      else
        blendhash["blendgreenchange"]=false
      end
      if sliderwin2.value(5)
        blendhash["blendbluechange"]=true
        blendhash["blendcolorblue"]=sliderwin2.value(5)
      else
        blendhash["blendbluechange"]=false
      end
      if sliderwin2.value(6)
        blendhash["blendalphachange"]=true
        blendhash["blendcoloralpha"]=sliderwin2.value(6)
      else
        blendhash["blendalphachange"]=false
      end
      break
    end
    if sliderwin2.changed?(cancelbutton) || Input.trigger?(Input::B)
...
def pbSelectSE(canvas,audio)
  ...
  animfiles = animfiles.sort_by{ |name| name.include?("[Play user's cry]") ? 0 : (name.include?(".wav") ? 1 : (name.include?(".mp3") ? 2 : (name.include?(".ogg") ? 3 : 4)))}
  ...
  loop do
    ...
    if Input.trigger?(Input::C) && animfiles.length>0
      ...
      ret = pbsearchSE(canvas,audio,animfiles)
      break
    end
  end
  cmdwin.dispose
...
def pbsearchSE(canvas,audio,animfiles)
  ...
  for i in 0...animfiles.length
    upcasednames = animfiles[i].upcase
    if upcasednames.include?(searchnameupcase)
      newanimations.push(animfiles[i])
    end
  end
  if newanimations.length == 0
    Kernel.pbMessage("no results!")
    return false
  end
  animfiles=newanimations
  cmdNone=0
  cmdwin=pbListWindow(animfiles,320)
  cmdwin.height=480
  cmdwin.opacity=200
...
def pbSelectBG(canvas,timing)
  ...
  animfiles.sort!
  animfiles.unshift(_INTL"[Erase background graphic]")
  cmdwin=pbListWindow(animfiles,320)
  cmdwin.height=480
  cmdwin.opacity=200
  cmdwin.viewport=canvas.viewport
  ...
  maxsizewindow.addSlider(_INTL("X:"),-5000,5000,timing.bgX || 0)
  maxsizewindow.addSlider(_INTL("Y:"),-5000,5000,timing.bgY || 0)
  ...
  loop do
    ...
    if Input.trigger?(Input::C) && animfiles.length>0
      ...
      maxsizewindow.dispose
      ret=pbSearchBG(animfiles,canvas,timing)
      break
    end
  end
  cmdwin.dispose
...
def pbSearchBG(animfiles,canvas,timing)
  window=ControlWindow.new(320,128,320,32*4)
  window.z=99999
  window.addControl(TextField.new(_INTL("Search for:"),""))
  okbutton=window.addButton(_INTL("OK"))
...
def pbEditBG(canvas,timing)
  ...
  maxsizewindow.addSlider(_INTL("Duration:"),0,100,timing.duration)
  maxsizewindow.addOptionalSlider(_INTL("X:"),-5000,5000,timing.bgX || 0)
  maxsizewindow.addOptionalSlider(_INTL("Y:"),-5000,5000,timing.bgY || 0)
...
def pbCopyCels(canvas)
  sliderwin2=ControlWindow.new(0,0,300,32*19)
  sliderwin2.viewport=canvas.viewport
  sliderwin2.opacity=200
  s1set0=sliderwin2.addSlider(_INTL("First Frame:"),1,canvas.animation.length,1)
  s1set1=sliderwin2.addSlider(_INTL("Last Frame:"),1,canvas.animation.length,canvas.animation.length)
  s1set2=sliderwin2.addSlider(_INTL("Copy From:"),0,PBAnimation::MAXSPRITES-1,0)
  s1set3=sliderwin2.addSlider(_INTL("Copy To:"),0,PBAnimation::MAXSPRITES-1,0)
  s1set4=sliderwin2.addSlider(_INTL("Frame Offset:"),0-canvas.animation.length,canvas.animation.length,0)
  set0=sliderwin2.addCheckbox(_INTL("Pattern"))
  set1=sliderwin2.addCheckbox(_INTL("Position"))
  set2=sliderwin2.addCheckbox(_INTL("Zoom"))
  set3=sliderwin2.addCheckbox(_INTL("Angle"))
  set4=sliderwin2.addCheckbox(_INTL("Opacity"))
  set5=sliderwin2.addCheckbox(_INTL("Blending"))
  set6=sliderwin2.addCheckbox(_INTL("Mirror"))
  set7=sliderwin2.addCheckbox(_INTL("Priority"))
  set8=sliderwin2.addCheckbox(_INTL("Focus"))
  set9=sliderwin2.addCheckbox(_INTL("Blend Color"))  
  set10=sliderwin2.addCheckbox(_INTL("Color Tone"))
  okbutton=sliderwin2.addButton(_INTL("OK"))
  cancelbutton=sliderwin2.addButton(_INTL("Cancel"))
  loop do
    Graphics.update
    ...
    if sliderwin2.changed?(okbutton) || Input.trigger?(Input::C)
      ...
      origincel=sliderwin2.value(s1set2)
      targetcel=sliderwin2.value(s1set3)
      clonearray=[] #Used so cells don't get overwritten mid-copy
      count=0 #To find position in clone array
      for i in startframe..endframe
        clonecell=canvas.animation[i][origincel]
        clonecell=clonecell.clone
        clonearray.push(clonecell)
      end
      for i in startframe..endframe
        offset=sliderwin2.value(s1set4) if s1set3
        offset=0 if !offset
        next if (i+offset)<0 || (i+offset)>(canvas.animation.length-1)
        copycel=clonearray[count]
        pastecel=canvas.animation[i+offset][targetcel]
        next if !copycel || !pastecel
        pastecel[AnimFrame::PATTERN]=copycel[AnimFrame::PATTERN] if sliderwin2.value(set0)
        if sliderwin2.value(set1)
          pastecel[AnimFrame::X]=copycel[AnimFrame::X]
          pastecel[AnimFrame::Y]=copycel[AnimFrame::Y]
        end
        if sliderwin2.value(set2)
          pastecel[AnimFrame::ZOOMX]=copycel[AnimFrame::ZOOMX]
          pastecel[AnimFrame::ZOOMY]=copycel[AnimFrame::ZOOMY]
        end
        pastecel[AnimFrame::ANGLE]=copycel[AnimFrame::ANGLE] if sliderwin2.value(set3)
        pastecel[AnimFrame::OPACITY]=copycel[AnimFrame::OPACITY] if sliderwin2.value(set4)
        pastecel[AnimFrame::BLENDTYPE]=copycel[AnimFrame::BLENDTYPE] if sliderwin2.value(set5)
        pastecel[AnimFrame::MIRROR]=copycel[AnimFrame::MIRROR] if sliderwin2.value(set6)
        pastecel[AnimFrame::PRIORITY]=copycel[AnimFrame::PRIORITY] if sliderwin2.value(set7)
        pastecel[AnimFrame::FOCUS]=copycel[AnimFrame::FOCUS] if sliderwin2.value(set8)
        if sliderwin2.value(set9)
          pastecel[AnimFrame::COLORRED]=copycel[AnimFrame::COLORRED]
          pastecel[AnimFrame::COLORGREEN]=copycel[AnimFrame::COLORGREEN]
          pastecel[AnimFrame::COLORBLUE]=copycel[AnimFrame::COLORBLUE]
          pastecel[AnimFrame::COLORALPHA]=copycel[AnimFrame::COLORALPHA]
        end
        if sliderwin2.value(set10)
          pastecel[AnimFrame::TONERED]=copycel[AnimFrame::TONERED]
          pastecel[AnimFrame::TONEGREEN]=copycel[AnimFrame::TONEGREEN]
          pastecel[AnimFrame::TONEBLUE]=copycel[AnimFrame::TONEBLUE]
          pastecel[AnimFrame::TONEGRAY]=copycel[AnimFrame::TONEGRAY]
        end
        count+=1
      end
      canvas.invalidate
...
def pbTweening(canvas)
  ...
  set3=sliderwin2.addCheckbox(_INTL("Tone/Colour"))
  okbutton=sliderwin2.addButton(_INTL("OK"))
  cancelbutton=sliderwin2.addButton(_INTL("Cancel"))
  loop do
    Graphics.update
    ...
    if sliderwin2.changed?(okbutton) || Input.trigger?(Input::C)
      ...
      for j in startcel..endcel
        ...
        diffRedTone=cel2[AnimFrame::TONERED]-cel1[AnimFrame::TONERED]
        diffBlueTone=cel2[AnimFrame::TONEBLUE]-cel1[AnimFrame::TONEBLUE]
        diffGreenTone=cel2[AnimFrame::TONEGREEN]-cel1[AnimFrame::TONEGREEN]
        diffGrayTone=cel2[AnimFrame::TONEGRAY]-cel1[AnimFrame::TONEGRAY]
        diffRedColor=cel2[AnimFrame::COLORRED]-cel1[AnimFrame::COLORRED]
        diffBlueColor=cel2[AnimFrame::COLORBLUE]-cel1[AnimFrame::COLORBLUE]
        diffGreenColor=cel2[AnimFrame::COLORGREEN]-cel1[AnimFrame::COLORGREEN]
        diffAlphaColor=cel2[AnimFrame::COLORALPHA]-cel1[AnimFrame::COLORALPHA]        
        ...
        startRedTone=cel1[AnimFrame::TONERED]        
        startBlueTone=cel1[AnimFrame::TONEBLUE]        
        startGreenTone=cel1[AnimFrame::TONEGREEN]        
        startGrayTone=cel1[AnimFrame::TONEGRAY]      
        startRedColor=cel1[AnimFrame::COLORRED]
        startBlueColor=cel1[AnimFrame::COLORBLUE]
        startGreenColor=cel1[AnimFrame::COLORGREEN]
        startAlphaColor=cel1[AnimFrame::COLORALPHA]         
        for k in 0..frames
          ...
          if sliderwin2.value(set3) #|| !curcel # I think this needs to be commented?
            cel[AnimFrame::TONERED]=startRedTone+(diffRedTone*k/frames)
            cel[AnimFrame::TONEBLUE]=startBlueTone+(diffBlueTone*k/frames)
            cel[AnimFrame::TONEGREEN]=startGreenTone+(diffGreenTone*k/frames)
            cel[AnimFrame::TONEGRAY]=startGrayTone+(diffGrayTone*k/frames)
            cel[AnimFrame::COLORRED]=startRedColor+(diffRedColor*k/frames)
            cel[AnimFrame::COLORBLUE]=startBlueColor+(diffBlueColor*k/frames)
            cel[AnimFrame::COLORGREEN]=startGreenColor+(diffGreenColor*k/frames)
            cel[AnimFrame::COLORALPHA]=startAlphaColor+(diffAlphaColor*k/frames)            
          end          
            cel[AnimFrame::FOCUS]=cel1[AnimFrame::FOCUS]         
        end
      end
      canvas.invalidate
      break
...
def pbReverseAnim(canvas)
  ...
  okbutton=sliderwin1.addButton(_INTL("OK"))
  cancelbutton=sliderwin1.addButton(_INTL("Cancel"))
  loop do
    Graphics.update
    Input.update
    sliderwin1.update
    if sliderwin1.changed?(okbutton) || Input.trigger?(Input::C)
      startframe=sliderwin1.value(s1set0)-1
      endframe=sliderwin1.value(s1set1)-1
      if startframe>endframe
        startframe=sliderwin1.value(s1set1)-1
        endframe=sliderwin1.value(s1set0)-1
      end
      midpoint=endframe-((endframe-startframe)/2)
      for i in startframe..endframe
        break if (i>midpoint || i==midpoint)
        count=i-startframe
        canvas.copyFrame(i,endframe-count,true)
      end
      canvas.invalidate
      break
    end
    if sliderwin1.changed?(cancelbutton) || Input.trigger?(Input::B)
      break
    end
  end
  sliderwin1.dispose
...
def pbFlipAroundPoint(canvas)
  sliderwin1=ControlWindow.new(0,0,300,32*11)
  sliderwin1.viewport=canvas.viewport
  sliderwin1.opacity=200
  s1set0=sliderwin1.addSlider(_INTL("First Frame:"),1,canvas.animation.length,1)
  s1set1=sliderwin1.addSlider(_INTL("Last Frame:"),1,canvas.animation.length,canvas.animation.length)
  ...
  s1set4=sliderwin1.addSlider(_INTL("X Flip Point:"),-64,512+64,384)
  s1set5=sliderwin1.addSlider(_INTL("Y Flip Point:"),-64,384+64,96)
  set6=sliderwin1.addCheckbox(_INTL("Flip Vertical"))
  set7=sliderwin1.addCheckbox(_INTL("Flip Horizontal"))
  okbutton=sliderwin1.addButton(_INTL("OK"))
  cancelbutton=sliderwin1.addButton(_INTL("Cancel"))
  loop do
    Graphics.update
    ...
    if sliderwin1.changed?(okbutton) || Input.trigger?(Input::C)
      ...
      xflip=sliderwin1.value(s1set4)
      yflip=sliderwin1.value(s1set5)
      for i in startframe..endframe
        for j in startcel..endcel
          next if !canvas.animation[i][j]
          cel=canvas.animation[i][j]
          if sliderwin1.value(set7)
            xflipnum=xflip-cel[AnimFrame::X]
            cel[AnimFrame::X]=xflip+xflipnum
          end
          if sliderwin1.value(6)
            yflipnum=yflip-cel[AnimFrame::Y]
            cel[AnimFrame::Y]=yflip+yflipnum
          end
        end
      end
      canvas.invalidate
...
def pbCelRotateFlip(canvas)
  sliderwin1=ControlWindow.new(0,0,300,32*7)
  sliderwin1.viewport=canvas.viewport
  sliderwin1.opacity=200
  s1set0=sliderwin1.addSlider(_INTL("First Frame:"),1,canvas.animation.length,1)
  s1set1=sliderwin1.addSlider(_INTL("Last Frame:"),1,canvas.animation.length,canvas.animation.length)
    ...
    if sliderwin1.changed?(okbutton) || Input.trigger?(Input::C)
      ...
      for i in startframe..endframe
        for j in startcel..endcel
          next if !canvas.animation[i][j]
          cel=canvas.animation[i][j]
          cel[AnimFrame::ANGLE]=360-cel[AnimFrame::ANGLE]
        end
      end
      canvas.invalidate
      break
...
def pbCellBatch(canvas)
  ...
  s1set2=sliderwin1.addSlider(_INTL("First Cel:"),0,PBAnimation::MAXSPRITES-1,0)
  s1set3=sliderwin1.addSlider(_INTL("Last Cel:"),0,PBAnimation::MAXSPRITES-1,PBAnimation::MAXSPRITES-1)
  sliderwin2=ControlWindow.new(300,0,340,32*17)
  sliderwin2.viewport=canvas.viewport
  sliderwin2.opacity=200
  set0=sliderwin2.addOptionalSlider(_INTL("Pattern:"),-2,1000,0)
  set11=sliderwin2.addOptionalSlider(_INTL("Increase Pattern:"),-1000,1000,0)
  ...
  blendhash={
    "blendcolorred"=>0,
    "blendredchange"=>false,
    "blendcolorgreen"=>0,
    "blendgreenchange"=>false,
    "blendcolorblue"=>0,
    "blendbluechange"=>false,
    "blendcoloralpha"=>0,
    "blendalphachange"=>false}
  tonehash={
    "colortonered"=>0,
    "toneredchange"=>false,
    "colortonegreen"=>0,
    "tonegreenchange"=>false,
    "colortoneblue"=>0,
    "tonebluechange"=>false,
    "colortonegray"=>0,
    "tonegraychange"=>false,}
  loop do
    Graphics.update
    Input.update
    sliderwin1.update
    sliderwin2.update
    if sliderwin2.changed?(flashbutton)
      pbSetFlashBatch(blendhash)
    end
    if sliderwin2.changed?(tonebutton)
      pbSetToneBatch(tonehash)
    end
    if sliderwin2.changed?(okbutton) || Input.trigger?(Input::C)
      startframe=sliderwin1.value(s1set0)-1
      endframe=sliderwin1.value(s1set1)-1
      ...
      for i in startframe..endframe
        for j in startcel..endcel
          ...
          cel[AnimFrame::PATTERN]+=sliderwin2.value(set11) if sliderwin2.value(set11)
          ...
          cel[AnimFrame::COLORRED]=blendhash["blendcolorred"] if blendhash["blendredchange"]
          cel[AnimFrame::COLORGREEN]=blendhash["blendcolorgreen"] if blendhash["blendgreenchange"]
          cel[AnimFrame::COLORBLUE]=blendhash["blendcolorblue"] if blendhash["blendbluechange"]
          cel[AnimFrame::COLORALPHA]=blendhash["blendcoloralpha"] if blendhash["blendalphachange"]
          cel[AnimFrame::TONERED]=tonehash["colortonered"] if tonehash["toneredchange"]
          cel[AnimFrame::TONEGREEN]=tonehash["colortonegreen"] if tonehash["tonegreenchange"]
          cel[AnimFrame::TONEBLUE]=tonehash["colortoneblue"] if tonehash["tonebluechange"]
          cel[AnimFrame::TONEGRAY]=tonehash["colortonegray"] if tonehash["tonegraychange"]
        end
      end
      canvas.invalidate
      break
...
def pbSelectAnim(canvas,animwin)
  ...
  loop do
    ...
    if Input.trigger?(Input::C) && animfiles.length>0
      ...
      pbSearchAnimSheet(canvas,animwin,animfiles)
      break
    end
  end
  bmpwin.dispose
...
def pbSearchAnimSheet(canvas,animwin,animfiles)
  window=ControlWindow.new(320,128,320,32*4)
  window.z=99999
  window.addControl(TextField.new(_INTL("Search for:"),""))
  okbutton=window.addButton(_INTL("OK"))
...
def pbSearchAnimations(animations,cmdwin,canvas,animwin)
  ...
  commands=[]
  for i in 0...hassearchterm.length
...
def tryLoadData(file)
  begin
    ...
    print("error with loading animations. tell cass or someone.")
    return nil
  end
end
...
#     }
      ...
      for i in canvas.currentframe...neededsize
        ...
        if @path # In case you try to click ok without defining anything
          cel[AnimFrame::X]=(path[i-canvas.currentframe][0]) - 64
          cel[AnimFrame::Y]=(path[i-canvas.currentframe][1]) - 64
        end
      end
...
def animationEditorMain(animation)
  ...
  sidewin.addButton(_INTL("Reverse..."))
  ...
  sidewin.addButton(_INTL("Targets"))
  sidewin.viewport=canvas.viewport
  # Bottom left menu
  sliderwin=ControlWindow.new(0,502,240,160)
  ...
  loop do
    ...
    if sidewin.changed?(9)
      pbReverseAnim(canvas)
    end
    ...
    if sidewin.changed?(14)
      positions=[_INTL("Single Target"),_INTL("Both Opponents"),_INTL("All Targets"),_INTL("Hide Partners")]
      for i in 0...positions.length
        selected="[ ]"
        if animation[animation.selected].targets==i
          selected="[x]"
        end
        positions[i]=sprintf("%s %s",selected,positions[i])
      end
      pos=Kernel.pbShowCommands(nil,positions,-1)
      if pos>=0
        animation[animation.selected].targets=pos
        canvas.update
      end
    end
    #if sidewin.changed?(14)
...
def animCompressor(animfile)
  framecount = 0
  nilsremoved = 0
  for i in 0...animfile.length
    for j in (1...animfile[i].length).to_a.reverse
      for k in 0...animfile[i][j].length
        curframe = animfile[i][j][k]
      ...
      for k in (0...animfile[i][j].length).to_a.reverse
        break if animfile[i][j][k] != nil
        animfile[i][j].delete_at(k)
        nilsremoved+=1
      end
    end
  end
