...
=begin disable replanting mechanic
      if berryData[0]>=5
        # Advance time
        timenow=pbGetTimeNow
        timeDiff=(timenow.to_i-berryData[3]) # in seconds
...
def pbBerryPlant
  ...
  watering=[]
  watering.push(PBItems::SPRAYDUCK) if hasConst?(PBItems,:SPRAYDUCK)
  watering.push(PBItems::SQUIRTBOTTLE) if hasConst?(PBItems,:SQUIRTBOTTLE)
  watering.push(PBItems::WAILMERPAIL) if hasConst?(PBItems,:WAILMERPAIL)
  berry=berryData[1]
...
=begin  
  # plant growing immediately ###change_item###
  if berryData[0] != 0 && berryData[0] != 5 
    if $PokemonBag.pbQuantity(PBItems::HONEY) > 0
      if Kernel.pbConfirmMessage(_INTL("Would you like to use {1} to grow the berry bush immediately?", PBItems.getName(PBItems::HONEY)))
        berryData[0]=5
        interp.setVariable(berryData)
        print thisEvent.direction
        thisEvent.move_down
        print thisEvent.direction
        thisEvent.update_pattern
        $PokemonBag.pbDeleteItem(PBItems::HONEY)
      end
    end
  end
=end
