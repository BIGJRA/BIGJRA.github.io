...
=end
  ...
  def pbGetDexList()
    ...
    for i in 1...regionalSpecies.length
      ...
      if pbCanAddForModeList?($PokemonGlobal.pokedexMode,nationalSpecies)
        height=$cache.pkmn_dex[nationalSpecies][14]
        weight=$cache.pkmn_dex[nationalSpecies][15]
  ...
  def pbRefreshDexList(index=0)
    ...
    monbitmap = pbPokemonBitmap(iconspecies,false)
    if monbitmap
  ...
  def pbSearchDexList(params)
    ...
    if params[1]!=0 # Filter by color
      dexlist=dexlist.find_all {|item|
         next false if !$Trainer.seen[item[0]]
         color=$cache.pkmn_dex[item[0]][:Color]
    ...
    if params[2]!=0 || params[3]!=0 # Filter by type
      ...
      dexlist=dexlist.find_all {|item|
         ...
         type1=$cache.pkmn_dex[item[0]][:Type1]
         type2=$cache.pkmn_dex[item[0]][:Type2]
  ...
  def pbChangeToDexEntry(species)
    ...
    if $Trainer.owned[species]
      ...
      height=$cache.pkmn_dex[species][:Height]
      weight=$cache.pkmn_dex[species][:Weight]
      ...
      if formPoke.form != 0
        ...
        if formnames != nil
          ...
          if thisform
            type1 = PokemonForms.dig(species,thisform,:Type1) if PokemonForms.dig(species,thisform,:Type1)
            type2 = PokemonForms.dig(species,thisform,:Type2) if PokemonForms.dig(species,thisform,:Type2)
            height = PokemonForms.dig(species,thisform,:Height) if PokemonForms.dig(species,thisform,:Height)
            weight = PokemonForms.dig(species,thisform,:Weight) if PokemonForms.dig(species,thisform,:Weight)
            dexentry = PokemonForms.dig(species,thisform,:DexEntry) if PokemonForms.dig(species,thisform,:DexEntry)
          end
        end
      end
...
=end
    ...
    pkmnbitmap=pbPokemonBitmap(formPoke.species)
    if formPoke.species==PBSpecies::EXEGGUTOR && formPoke.form==1
      @sprites["overlay"].bitmap.blt( 40-(pkmnbitmap.width-128)/2, (70-(pkmnbitmap.height-128)/2)+6, pkmnbitmap,pkmnbitmap.rect)
    else
