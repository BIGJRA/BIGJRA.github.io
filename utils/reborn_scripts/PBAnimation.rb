...
class PBAnimationPlayerX
  ...
  def initialize(animation,user,target,scene=nil,oppmove=false,ineditor=false)
    ...
    @partnerbs = (!ineditor && @realtarget && @realtarget.pbPartner && @realtarget.pbPartner.hp >0 &&
      !@realtarget.pbPartner.vanished && @realtarget!=@realuser && @animation.targets && @animation.targets==1)
    if @partnerbs
      ...
      srcline = [PokeBattle_SceneConstants::FOCUSUSER_X,PokeBattle_SceneConstants::FOCUSUSER_Y,
        PokeBattle_SceneConstants::FOCUSTARGET_X,PokeBattle_SceneConstants::FOCUSTARGET_Y]
      ...
      @centerboth = @realuser.index%2==0 ? useroutofnames + centerbothmons : centerbothmons + useroutofnames
    end
    allmontargets=[]
    if !ineditor && @animation.targets == 2
      for index in 0...4
        allmontargets.push(index) if index != @realuser.index && index != @realtarget.index &&
          user.battle.battlers[index].hp >0 && !user.battle.battlers[index].vanished
      end
  ...
  def update
    ...
    if (@frame&1)==0
          ...
          when 3   # Focused on user and target
            if @srcLine && @dstLine
              if @partnerbs && cel[AnimFrame::PATTERN]!=-2 && cel[AnimFrame::PATTERN]!=-1
                ...
                sprite.x=point[0]
                sprite.y=point[1]
                if isReversed(@srcLine[0],@srcLine[2],@centerboth[0],@centerboth[2]) && cel[AnimFrame::PATTERN]>=0
                  # Reverse direction
                ...
                sprite.x=point[0]
                sprite.y=point[1]
                if isReversed(@srcLine[0],@srcLine[2],@dstLine[0],@dstLine[2]) && cel[AnimFrame::PATTERN]>=0
                  # Reverse direction
                  sprite.mirror=!sprite.mirror
                end
              end
      ...
      if @partnerbs
        cel = thisframe[(@realtarget.index)-2] if @realtarget.index>1
        cel = thisframe[@realtarget.index] if @realtarget.index<2 #cel of target
        pbSpriteSetAnimFrame(@animpartner,cel,@usersprite,@animpartner)
        case cel[AnimFrame::FOCUS]
          when 1   # Focused on target
            if @realtarget.index==1 || (@realtarget.index-2)==1
              @animpartner.x=cel[AnimFrame::X]+@partnerOrig[0]-PokeBattle_SceneConstants::FOCUSTARGET_X
              @animpartner.y=cel[AnimFrame::Y]+@partnerOrig[1]-PokeBattle_SceneConstants::FOCUSTARGET_Y
            else
              @animpartner.x=cel[AnimFrame::X]+@userOrig[0]-PokeBattle_SceneConstants::FOCUSTARGET_X
              @animpartner.y=cel[AnimFrame::Y]+@userOrig[1]-PokeBattle_SceneConstants::FOCUSTARGET_Y
            end   
            when 2   # Focused on user
              if @realtarget.index==1 || (@realtarget.index-2)==1
                @animpartner.x=cel[AnimFrame::X]+@userOrig[0]-PokeBattle_SceneConstants::FOCUSUSER_X
                @animpartner.y=cel[AnimFrame::Y]+@userOrig[1]-PokeBattle_SceneConstants::FOCUSUSER_Y
              else
                @animpartner.x=cel[AnimFrame::X]+@partnerOrig[0]-PokeBattle_SceneConstants::FOCUSUSER_X
                @animpartner.y=cel[AnimFrame::Y]+@partnerOrig[1]-PokeBattle_SceneConstants::FOCUSUSER_Y
              end
            when 3   # Focused on user and target
              point=transformPoint(
      ...
      if @allmonbs
        ...
        targetbullshit+=1 if @realtarget.index==0 || @realtarget.index==2
        ...
        case cel[AnimFrame::FOCUS]
          when 1   # Focused on target
            if targetbullshit==1 || (targetbullshit-2)==1
              @target1sprite.x=cel[AnimFrame::X]+@target1Orig[0]-PokeBattle_SceneConstants::FOCUSTARGET_X
              @target1sprite.y=cel[AnimFrame::Y]+@target1Orig[1]-PokeBattle_SceneConstants::FOCUSTARGET_Y
            else
              @target1sprite.x=cel[AnimFrame::X]+@userOrig[0]-PokeBattle_SceneConstants::FOCUSTARGET_X
              @target1sprite.y=cel[AnimFrame::Y]+@userOrig[1]-PokeBattle_SceneConstants::FOCUSTARGET_Y
            end   
            when 2   # Focused on user
              if targetbullshit==1 || (targetbullshit-2)==1
                @target1sprite.x=cel[AnimFrame::X]+@userOrig[0]-PokeBattle_SceneConstants::FOCUSUSER_X
                @target1sprite.y=cel[AnimFrame::Y]+@userOrig[1]-PokeBattle_SceneConstants::FOCUSUSER_Y
              else
                @target1sprite.x=cel[AnimFrame::X]+@target1Orig[0]-PokeBattle_SceneConstants::FOCUSUSER_X
                @target1sprite.y=cel[AnimFrame::Y]+@target1Orig[1]-PokeBattle_SceneConstants::FOCUSUSER_Y
              end
            when 3   # Focused on user and target
              point=transformPoint(
        ...
        if @target2sprite
          ...
          case cel[AnimFrame::FOCUS]
           when 1   # Focused on target
              if targetbullshit==1 || (targetbullshit-2)==1
                @target2sprite.x=cel[AnimFrame::X]+@target2Orig[0]-PokeBattle_SceneConstants::FOCUSTARGET_X
                @target2sprite.y=cel[AnimFrame::Y]+@target2Orig[1]-PokeBattle_SceneConstants::FOCUSTARGET_Y
              else
                @target2sprite.x=cel[AnimFrame::X]+@userOrig[0]-PokeBattle_SceneConstants::FOCUSTARGET_X
                @target2sprite.y=cel[AnimFrame::Y]+@userOrig[1]-PokeBattle_SceneConstants::FOCUSTARGET_Y
              end   
              when 2   # Focused on user
                if targetbullshit==1 || (targetbullshit-2)==1
                  @target2sprite.x=cel[AnimFrame::X]+@userOrig[0]-PokeBattle_SceneConstants::FOCUSUSER_X
                  @target2sprite.y=cel[AnimFrame::Y]+@userOrig[1]-PokeBattle_SceneConstants::FOCUSUSER_Y
                else
                  @target2sprite.x=cel[AnimFrame::X]+@target2Orig[0]-PokeBattle_SceneConstants::FOCUSUSER_X
                  @target2sprite.y=cel[AnimFrame::Y]+@target2Orig[1]-PokeBattle_SceneConstants::FOCUSUSER_Y
                end
              when 3   # Focused on user and target
                point=transformPoint(
